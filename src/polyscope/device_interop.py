...
def resolve_default_device_interop_funcs() -> None:
    ...
    def get_array_from_unknown_data(arr):
        ...
        if arr_info_dict is None:
            raise ValueError(
                "Cannot read from GPU data object. The object must implement the __cuda_array_interface__, or if the cupy package is installed, implement the __dlpack__ protocol. Are you sure you are passing a GPU array?"
            )
    ...
    func_dict = {
        ...
        "register_gl_buffer": lambda native_id: check_cudart_err(
            cuda.bindings.runtime.cudaGraphicsGLRegisterBuffer(
                native_id,
        ...
        "register_gl_image_2d": lambda native_id: check_cudart_err(
        ...
        "unregister_resource": lambda handle: check_cudart_err(
            cuda.bindings.runtime.cudaGraphicsUnregisterResource(handle)
        ),
        # returns array
        ...
        "get_array_ptr": lambda input_array: get_array_from_unknown_data(input_array),
        # as in cudaMemcpy
        "memcpy": lambda dst_ptr, src_ptr, size: check_cudart_err(
            cuda.bindings.runtime.cudaMemcpy(
                dst_ptr,
                src_ptr,
                size,
                cuda.bindings.runtime.cudaMemcpyKind.cudaMemcpyDeviceToDevice,
            )
        ),
        # as in cudaMemcpy2DToArray
        "memcpy_2d": lambda dst_ptr, src_ptr, width, height: check_cudart_err(
...
def check_device_module_availibility() -> None:
    ...
    if get_render_engine_backend_name() not in supported_backends:
        raise ValueError(
            f"This Polyscope functionality is not supported by the current rendering backend ({get_render_engine_backend_name()}. Supported backends: {','.join(supported_backends)}."
        )
...
class CUDAOpenGLMappedAttributeBuffer:
    ...
    def set_data_from_array(
        ...
        if arr_nbytes is not None and arr_nbytes != self.cuda_buffer_size:
            ...
            raise ValueError(
                f"Mapped buffer write has wrong size, expected {arr_nbytes} bytes but got {self.cuda_buffer_size} bytes. Could it be the wrong size/shape or wrong dtype?"
            )
        ...
        if self.buffer_type == psb.DeviceBufferType.texture2d:
            ...
            if (
                ...
                if arr_nbytes != expected_bytes:
                    ...
                    # are about to do below.
                    raise ValueError(
                        f"Mapped buffer write has wrong size, expected {arr_nbytes} bytes but got {expected_bytes} bytes. Could it be the wrong size/shape or wrong dtype?"
                    )

            device_interop_funcs["memcpy_2d"](
                self.cuda_buffer_array,
                arr_ptr,
                texture_dims[0] * entry_size_in_bytes,
                texture_dims[1],
            )
