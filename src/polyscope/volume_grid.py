...
class VolumeGrid(Structure):
    ...
    def __init__(
        ...
        if instance is not None:
            ...
            self.bound_instance = psb.register_volume_grid(name, glm3u(node_dims), glm3(bound_low), glm3(bound_high))
    ...
    def add_scalar_quantity(
        ...
        **scalar_args: Unpack[VolumeGridScalarQuantityArgs],
        ...
        if defined_on == "nodes":
            if values_arr.shape != tuple(self.get_grid_node_dim()):
                raise ValueError(f"'values' should be a {self.get_grid_node_dim()} array")

            q = self.bound_instance.add_node_scalar_quantity(
                name, values_arr.flatten("F"), to_enum(psb.DataType, datatype)
            )

        elif defined_on == "cells":
            if values_arr.shape != tuple(self.get_grid_cell_dim()):
                raise ValueError(f"'values' should be a {self.get_grid_cell_dim()} array")
