...
class ManagedBuffer:
    ...
    def check_ref_still_valid(self) -> None:
    ...
    def get_texture_size(self) -> tuple[int, ...]:
        self.check_ref_still_valid()
    ...
    def get_value(self, ind: int, indY: int | None = None, indZ: int | None = None) -> Any:
        self.check_ref_still_valid()
    ...
    def update_data(self, new_vals: ArrayLike) -> None:
        self.check_ref_still_valid()
        self.update_data_from_host(new_vals)

    def update_data_from_host(self, new_vals: ArrayLike) -> None:
        self.check_ref_still_valid()
        ...
        new_vals_arr = np.asarray(new_vals)
        self.bound_buffer.update_data(new_vals_arr)

    def update_data_from_device(self, new_vals_device: Any) -> None:
        self.check_ref_still_valid()

        mapped_buffer = self.get_mapped_buffer_CUDAOpenGL()

        if self.device_buffer_type == psb.DeviceBufferType.attribute:
            assert isinstance(mapped_buffer, CUDAOpenGLMappedAttributeBuffer)
    ...
    def get_mapped_buffer_CUDAOpenGL(self) -> CUDAOpenGLMappedAttributeBuffer | CUDAOpenGLMappedTextureBuffer:
        self.check_ref_still_valid()
    ...
    def release_mapped_buffer_CUDAOpenGL(self) -> None:
    ...
    def get_texture_native_id(self) -> int:
        if self.device_buffer_type == psb.DeviceBufferType.attribute:
    ...
    def get_attribute_native_id(self) -> int:
        if self.device_buffer_type != psb.DeviceBufferType.attribute:
