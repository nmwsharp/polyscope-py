...
class VolumeMesh(Structure):
    ...
    def check_index_array(self, arr: NDArray | None, dim: int, name: str) -> None:
        ...
        if (len(arr.shape) != 2) or (arr.shape[1] != dim):
            raise ValueError(
                "volume mesh {} array should have shape (N,{}); shape is {}".format(name, dim, str(arr.shape))
            )
    ...
    def add_scalar_quantity(
        ...
        **scalar_args: Any,
        ...
        if defined_on == "vertices":
            ...
            if values_arr.shape[0] != self.n_cells():
                raise ValueError("'values' should be a length n_cells array")
