...
def show(forFrames: Optional[int] = None) -> None:
...
### Screenshots
def screenshot(filename: Optional[str] = None, transparent_bg: bool = True) -> None:
...
def load_blendable_material(
    mat_name, filenames=None, filename_base=None, filename_ext=None
):
...
def str_to_navigate_style(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad navigate style specifier '{}', should be one of [{}]".format(
...
def str_to_projection_mode(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad projection mode specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_updir(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad up direction specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_datatype(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad datatype specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_vectortype(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad vectortype specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_param_coords_type(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad param coords type specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_param_viz_style(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad param viz style specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
    ...
    if s not in d_back_face_policy:
        raise ValueError(
            "Bad back face policy specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d_back_face_policy.keys()])
            )
        )
...
def back_face_policy_to_str(val):
    ...
    raise ValueError(
        "Bad back face policy specifier '{}', should be one of [{}]".format(
            val, ",".join(["'{}'".format(x) for x in d_back_face_policy.values()])
        )
    )
...
def str_to_ground_plane_mode(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad ground plane mode specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_transparency_mode(s):
    ...
    if s not in d:
        raise ValueError(
            "Bad transparenccy mode specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d.keys()])
            )
        )
...
def str_to_point_render_mode(s):
    ...
    if s not in d_point_render_mode:
        raise ValueError(
            "Bad point render mode specifier '{}', should be one of [{}]".format(
                s, ",".join(["'{}'".format(x) for x in d_point_render_mode.keys()])
            )
        )
...
def point_render_mode_to_str(val):
    ...
    raise ValueError(
        "Bad point render mode specifier '{}', should be one of [{}]".format(
            val, ",".join(["'{}'".format(x) for x in d_point_render_mode.values()])
        )
    )
