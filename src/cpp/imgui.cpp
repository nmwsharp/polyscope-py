...
void bind_imgui_funcs(py::module& m) {
    m.def("NewFrame", &ImGui::NewFrame, "Starts a new ImGui frame. This must be called before any other ImGui commands in a frame.");
m.def("EndFrame", &ImGui::EndFrame, "Ends the current ImGui frame. If you don't need to render data, you can call this without Render().");
...
m.def("ShowMetricsWindow", 
    [](std::optional<bool> open) {
        bool *p_open = open.has_value() ? &open.value() : nullptr;
        ImGui::ShowMetricsWindow(p_open);
        return open;
    }
    , py::arg("open") = std::nullopt, "Creates a Metrics/Debugger window that displays internal state and other metrics.");
...
m.def("ShowUserGuide", &ImGui::ShowUserGuide, "Shows a user guide window with basic help and information about ImGui usage.");
m.def("GetVersion", &ImGui::GetVersion, "Returns the ImGui version as a string.");
m.def("StyleColorsDark", 
    []() {
...
m.def("Begin", 
    ...
    , py::arg("name"), py::arg("open") = std::nullopt, py::arg("flags") = 0, "Begins a new window. Returns true if the window is visible.");
...
m.def("BeginChild", 
    ...
    , py::arg("str_id"), py::arg("size") = ImVec2(0, 0), py::arg("child_flags") = 0, py::arg("window_flags") = 0, "Begins a child window, which is a self-contained scrolling/clipping region.");
...
m.def("IsWindowFocused", &ImGui::IsWindowFocused, py::arg("flags") = 0, "Returns true if the current window is focused, or its root/child depending on flags.");
m.def("IsWindowHovered", &ImGui::IsWindowHovered, py::arg("flags") = 0, "Returns true if the current window is hovered and hoverable.");
...
m.def("SetNextWindowPos", &ImGui::SetNextWindowPos, py::arg("pos"), py::arg("cond") = 0, py::arg("pivot") = ImVec2(0, 0), "Sets the position for the next window. Call before Begin().");
m.def("SetNextWindowSize", &ImGui::SetNextWindowSize, py::arg("size"), py::arg("cond") = 0, "Sets the size for the next window. Call before Begin().");
m.def("SetNextWindowContentSize", &ImGui::SetNextWindowContentSize, py::arg("size"), "Sets the content size for the next window (scrollable client area). This does not include window decorations like the title bar or menu bar.");
m.def("SetNextWindowCollapsed", &ImGui::SetNextWindowCollapsed, py::arg("collapsed"), py::arg("cond") = 0, "Sets the collapsed state for the next window. This should be called before Begin().");
m.def("SetNextWindowFocus", &ImGui::SetNextWindowFocus, "Sets the next window to be focused and top-most. This should be called before Begin().");
m.def("SetNextWindowScroll", &ImGui::SetNextWindowScroll, py::arg("scroll"), "Sets the scrolling position for the next window. Use < 0.0f for an axis to leave it unchanged.");
m.def("SetNextWindowBgAlpha", &ImGui::SetNextWindowBgAlpha, py::arg("alpha"), "Sets the background color alpha for the next window. This can be used to override the alpha component of ImGuiCol_WindowBg/ChildBg/PopupBg.");
m.def("SetWindowPos", 
    ...
    , py::arg("pos"), py::arg("cond") = 0, "(Not recommended) Sets the current window position. This should be called within a Begin()/End() block. Prefer using SetNextWindowPos().");
m.def("SetWindowSize", 
    ...
    , py::arg("size"), py::arg("cond") = 0, "(Not recommended) Sets the current window size. This should be called within a Begin()/End() block. Prefer using SetNextWindowSize().");
m.def("SetWindowCollapsed", 
    [](bool collapsed, ImGuiCond cond) {
        
        ImGui::SetWindowCollapsed(collapsed, cond);
        
    }
    , py::arg("collapsed"), py::arg("cond") = 0, "(Not recommended) Sets the current window collapsed state. Prefer using SetNextWindowCollapsed().");
m.def("SetWindowFocus", 
    []() {
...
m.def("SetWindowSize", 
    ...
    , py::arg("name"), py::arg("size"), py::arg("cond") = 0, "Sets the size of the window specified by name. Set an axis to 0.0f to force an auto-fit.");
m.def("SetWindowCollapsed", 
    ...
    , py::arg("name"), py::arg("collapsed"), py::arg("cond") = 0, "Sets the collapsed state of the window specified by name.");
m.def("SetWindowFocus", 
    [](const char* name) {
...
m.def("GetScrollX", &ImGui::GetScrollX, "Returns the horizontal scrolling amount, ranging from 0 to GetScrollMaxX().");
m.def("GetScrollY", &ImGui::GetScrollY, "Returns the vertical scrolling amount, ranging from 0 to GetScrollMaxY().");
m.def("SetScrollX", &ImGui::SetScrollX, py::arg("scroll_x"), "Sets the horizontal scrolling amount, ranging from 0 to GetScrollMaxX().");
m.def("SetScrollY", &ImGui::SetScrollY, py::arg("scroll_y"), "Sets the vertical scrolling amount, ranging from 0 to GetScrollMaxY().");
m.def("GetScrollMaxX", &ImGui::GetScrollMaxX, "Returns the maximum horizontal scrolling amount, calculated as ContentSize.x - WindowSize.x - DecorationsSize.x.");
m.def("GetScrollMaxY", &ImGui::GetScrollMaxY, "Returns the maximum vertical scrolling amount, calculated as ContentSize.y - WindowSize.y - DecorationsSize.y.");
m.def("SetScrollHereX", &ImGui::SetScrollHereX, py::arg("center_x_ratio") = 0.5f, "Adjusts the horizontal scrolling amount to make the current cursor position visible. center_x_ratio=0.0 aligns left, 0.5 centers, 1.0 aligns right.");
m.def("SetScrollHereY", &ImGui::SetScrollHereY, py::arg("center_y_ratio") = 0.5f, "Adjusts the vertical scrolling amount to make the current cursor position visible. center_y_ratio=0.0 aligns top, 0.5 centers, 1.0 aligns bottom.");
m.def("SetScrollFromPosX", &ImGui::SetScrollFromPosX, py::arg("local_x"), py::arg("center_x_ratio") = 0.5f, "Adjusts the horizontal scrolling amount to make the specified position visible. Use GetCursorStartPos() + offset to compute a valid position.");
m.def("SetScrollFromPosY", &ImGui::SetScrollFromPosY, py::arg("local_y"), py::arg("center_y_ratio") = 0.5f, "Adjusts the vertical scrolling amount to make the specified position visible. Use GetCursorStartPos() + offset to compute a valid position.");
m.def("PushStyleColor", 
    [](ImGuiCol idx, ImU32 col) {
        
        ImGui::PushStyleColor(idx, col);
        
    }
    , py::arg("idx"), py::arg("col"), "Pushes a style color onto the style color stack, modifying the color specified by idx.");
m.def("PushStyleColor", 
    ...
    , py::arg("idx"), py::arg("col"), "Pushes a style color onto the style color stack, modifying the color specified by idx.");
m.def("PopStyleColor", &ImGui::PopStyleColor, py::arg("count") = 1, "Pops one or more style colors from the style color stack.");
m.def("PushStyleVar", 
    ...
    , py::arg("idx"), py::arg("val"), "Pushes a style variable onto the style variable stack, modifying the float variable specified by idx.");
m.def("PushStyleVar", 
    ...
    , py::arg("idx"), py::arg("val"), "Pushes a style variable onto the style variable stack, modifying the ImVec2 variable specified by idx.");
m.def("PopStyleVar", &ImGui::PopStyleVar, py::arg("count") = 1, "Pops one or more style variables from the style variable stack.");
m.def("PushItemFlag", &ImGui::PushItemFlag, py::arg("option"), py::arg("enabled"), "Pushes an item flag onto the item flag stack, modifying the specified option.");
m.def("PopItemFlag", &ImGui::PopItemFlag, "Pops the last item flag pushed onto the item flag stack.");
m.def("PushItemWidth", &ImGui::PushItemWidth, py::arg("item_width"), "Pushes the width of items for common large 'item+label' widgets. >0.0f: width in pixels, <0.0f aligns the width to the right of the window.");
m.def("PopItemWidth", &ImGui::PopItemWidth, "Pops the last item width pushed onto the item width stack.");
m.def("SetNextItemWidth", &ImGui::SetNextItemWidth, py::arg("item_width"), "Sets the width of the next common large 'item+label' widget. >0.0f: width in pixels, <0.0f aligns the width to the right of the window.");
m.def("CalcItemWidth", &ImGui::CalcItemWidth, "Calculates the width of the item given the pushed settings and current cursor position.");
m.def("PushTextWrapPos", &ImGui::PushTextWrapPos, py::arg("wrap_local_pos_x") = 0.0f, "Pushes the word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window; > 0.0f: wrap at 'wrap_pos_x' position in window local space.");
...
m.def("GetFontTexUvWhitePixel", &ImGui::GetFontTexUvWhitePixel, "Returns the UV coordinate for a white pixel, useful for drawing custom shapes via the ImDrawList API.");
m.def("GetColorU32", 
    ...
    , py::arg("idx"), py::arg("alpha_mul") = 1.0f, "Retrieves the style color specified by idx, with the style alpha applied and an optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList.");
m.def("GetColorU32", 
    [](const ImVec4& col) {
    ...
    , py::arg("col"), "Retrieves the given color with the style alpha applied, packed as a 32-bit value suitable for ImDrawList.");
m.def("GetColorU32", 
    ...
    , py::arg("col"), py::arg("alpha_mul") = 1.0f, "Retrieves the given color with the style alpha applied, packed as a 32-bit value suitable for ImDrawList.");
m.def("GetStyleColorVec4", &ImGui::GetStyleColorVec4, py::arg("idx"), "Retrieves the style color as stored in the ImGuiStyle structure. Use this to feed back into PushStyleColor(), otherwise use GetColorU32() to get the style color with style alpha baked in.");
m.def("GetCursorScreenPos", &ImGui::GetCursorScreenPos, "Returns the current cursor position in absolute screen coordinates. This is typically the best function to use for getting cursor position.");
m.def("SetCursorScreenPos", &ImGui::SetCursorScreenPos, py::arg("pos"), "Sets the cursor position in absolute screen coordinates.");
m.def("GetContentRegionAvail", &ImGui::GetContentRegionAvail, "Returns the available space from the current position. This is typically the best function to use for getting the available content region.");
m.def("GetCursorPos", &ImGui::GetCursorPos, "Returns the current cursor position in window-local coordinates.");
m.def("GetCursorPosX", &ImGui::GetCursorPosX, "Returns the X coordinate of the current cursor position in window-local coordinates.");
m.def("GetCursorPosY", &ImGui::GetCursorPosY, "Returns the Y coordinate of the current cursor position in window-local coordinates.");
m.def("SetCursorPos", &ImGui::SetCursorPos, py::arg("local_pos"), "Sets the cursor position in window-local coordinates.");
m.def("SetCursorPosX", &ImGui::SetCursorPosX, py::arg("local_x"), "Sets the X coordinate of the cursor position in window-local coordinates.");
m.def("SetCursorPosY", &ImGui::SetCursorPosY, py::arg("local_y"), "Sets the Y coordinate of the cursor position in window-local coordinates.");
m.def("GetCursorStartPos", &ImGui::GetCursorStartPos, "Returns the initial cursor position in window-local coordinates. Call GetCursorScreenPos() after Begin() to get the absolute coordinates version.");
m.def("Separator", &ImGui::Separator, "Creates a separator, generally horizontal. Inside a menu bar or in horizontal layout mode, this becomes a vertical separator.");
m.def("SameLine", &ImGui::SameLine, py::arg("offset_from_start_x") = 0.0f, py::arg("spacing") = -1.0f, "Lays out widgets horizontally by undoing the last carriage return. The X position is given in window coordinates.");
m.def("NewLine", &ImGui::NewLine, "Forces a new line when in a horizontal-layout context or undoes a SameLine().");
m.def("Dummy", &ImGui::Dummy, py::arg("size"), "Adds a dummy item of the given size. Unlike InvisibleButton(), Dummy() won't take mouse clicks or be navigable.");
m.def("Unindent", &ImGui::Unindent, py::arg("indent_w") = 0.0f, "Moves the content position back to the left by indent_w, or by style.IndentSpacing if indent_w <= 0.");
m.def("BeginGroup", &ImGui::BeginGroup, "Locks the horizontal starting position for a group of items.");
m.def("EndGroup", &ImGui::EndGroup, "Unlocks the horizontal starting position for a group of items and captures the whole group bounding box as one 'item.'");
m.def("AlignTextToFramePadding", &ImGui::AlignTextToFramePadding, "Vertically aligns the upcoming text baseline to FramePadding.y, so that it aligns properly with regularly framed items.");
m.def("GetTextLineHeight", &ImGui::GetTextLineHeight, "Returns the height of a text line, approximately equal to FontSize.");
m.def("GetTextLineHeightWithSpacing", &ImGui::GetTextLineHeightWithSpacing, "Returns the height of a text line, including spacing. Approximately equal to FontSize + style.ItemSpacing.y.");
m.def("GetFrameHeight", &ImGui::GetFrameHeight, "Returns the height of a frame, approximately equal to FontSize + style.FramePadding.y * 2.");
m.def("GetFrameHeightWithSpacing", &ImGui::GetFrameHeightWithSpacing, "Returns the height of a frame, including spacing. Approximately equal to FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y.");
m.def("PushID", 
    [](const char* str_id) {
...
m.def("GetID", 
    ...
    , py::arg("str_id"), "Calculates a unique ID by hashing the entire ID stack with the given parameter. Useful for querying ImGuiStorage.");
m.def("GetID", 
    [](const char* str_id_begin, const char* str_id_end) {
    ...
    , py::arg("str_id_begin"), py::arg("str_id_end"), "Calculates a unique ID by hashing the entire ID stack with the given substring.");
m.def("GetID", 
    [](int int_id) {
...
m.def("TextWrapped", 
    ...
    , py::arg("text"), "Displays formatted text with word-wrapping enabled. The text will wrap at the end of the window or column by default.");
m.def("LabelText", 
...
m.def("SeparatorText", &ImGui::SeparatorText, py::arg("label"), "Displays formatted text with a horizontal line separator.");
m.def("Button", &ImGui::Button, py::arg("label"), py::arg("size") = ImVec2(0, 0), "Creates a button with the specified label and size. Returns true when the button is pressed.");
m.def("SmallButton", &ImGui::SmallButton, py::arg("label"), "Creates a small button with the specified label. Useful for embedding within text.");
m.def("InvisibleButton", &ImGui::InvisibleButton, py::arg("str_id"), py::arg("size"), py::arg("flags") = 0, "Creates an invisible button with flexible behavior, frequently used to build custom behaviors using the public API (along with IsItemActive, IsItemHovered, etc.).");
m.def("ArrowButton", &ImGui::ArrowButton, py::arg("str_id"), py::arg("dir"), "Creates a square button with an arrow shape pointing in the specified direction.");
m.def("Checkbox", 
    ...
    , py::arg("label"), py::arg("v"), "Creates a checkbox with the specified label and boolean value. Returns true when the checkbox is clicked.");
m.def("CheckboxFlags", 
    ...
    , py::arg("label"), py::arg("flags"), py::arg("flags_value"), "Creates a checkbox that toggles specific flags within an integer.");
m.def("RadioButton", 
...
m.def("RadioButton", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_button"), "Creates a radio button that sets the specified integer value when clicked.");
m.def("ProgressBar", 
    [](float fraction, const ImVec2& size_arg, const std::optional<std::string>& overlay) {
        const char *overlay_ = overlay.has_value() ? overlay.c_str() : nullptr;
        ImGui::ProgressBar(fraction, size_arg, overlay_);
        
    }
    , py::arg("fraction"), py::arg("size_arg") = ImVec2(-FLT_MIN, 0), py::arg("overlay") = std::nullopt, "Creates a progress bar with the specified fraction of completion, size, and optional overlay text.");
m.def("Bullet", &ImGui::Bullet, "Draws a small circle and keeps the cursor on the same line. Advances cursor x position by GetTreeNodeToLabelSpacing(), the same distance that TreeNode() uses.");
m.def("TextLink", &ImGui::TextLink, py::arg("label"), "Creates a hyperlink-style text button that returns true when clicked.");
m.def("TextLinkOpenURL", 
    [](const char* label, const std::optional<std::string>& url) {
        const char *url_ = url.has_value() ? url.c_str() : nullptr;
        ImGui::TextLinkOpenURL(label, url_);
        
    }
    , py::arg("label"), py::arg("url") = std::nullopt, "Creates a hyperlink-style text button that automatically opens the specified URL when clicked.");
m.def("BeginCombo", &ImGui::BeginCombo, py::arg("label"), py::arg("preview_value"), py::arg("flags") = 0, "Begins a combo box (dropdown) with the specified label and preview value. Returns true if the combo box is open.");
...
const auto items_count = items_.size();
    ...
    , py::arg("label"), py::arg("current_item"), py::arg("items"), py::arg("popup_max_height_in_items") = -1, "Creates a combo box (dropdown) with the specified label, current item index, and items array. Returns true if the selection is changed.");
m.def("Combo", 
    ...
    , py::arg("label"), py::arg("current_item"), py::arg("items_separated_by_zeros"), py::arg("popup_max_height_in_items") = -1, "Creates a combo box (dropdown) with items separated by null characters. Returns true if the selection is changed.");
m.def("DragFloat", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0.0f, py::arg("v_max") = 0.0f, py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a draggable float slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragFloat2", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0.0f, py::arg("v_max") = 0.0f, py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a draggable float2 slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragFloat3", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0.0f, py::arg("v_max") = 0.0f, py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a draggable float3 slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragFloat4", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0.0f, py::arg("v_max") = 0.0f, py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a draggable float4 slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragFloatRange2", 
    [](const char* label, float v_current_min, float v_current_max, float v_speed, float v_min, float v_max, const char* format, const std::optional<std::string>& format_max, ImGuiSliderFlags flags) {
        float *v_current_min_ = &v_current_min;
float *v_current_max_ = &v_current_max;
const char *format_max_ = format_max.has_value() ? format_max.c_str() : nullptr;
        const auto res_ = ImGui::DragFloatRange2(label, v_current_min_, v_current_max_, v_speed, v_min, v_max, format, format_max_, flags);
        return std::make_tuple(res_, v_current_min, v_current_max);
    }
    , py::arg("label"), py::arg("v_current_min"), py::arg("v_current_max"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0.0f, py::arg("v_max") = 0.0f, py::arg("format") = "%.3f", py::arg("format_max") = std::nullopt, py::arg("flags") = 0, "Creates a draggable float range slider with the specified label, current min/max values, speed, minimum/maximum values, format strings, and optional flags. Returns true if the values are changed.");
m.def("DragInt", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0, py::arg("v_max") = 0, py::arg("format") = "%d", py::arg("flags") = 0, "Creates a draggable integer slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragInt2", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0, py::arg("v_max") = 0, py::arg("format") = "%d", py::arg("flags") = 0, "Creates a draggable int2 slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragInt3", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0, py::arg("v_max") = 0, py::arg("format") = "%d", py::arg("flags") = 0, "Creates a draggable int3 slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragInt4", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0, py::arg("v_max") = 0, py::arg("format") = "%d", py::arg("flags") = 0, "Creates a draggable int4 slider with the specified label, value, speed, minimum/maximum values, format string, and optional flags. Returns true if the value is changed.");
m.def("DragIntRange2", 
    [](const char* label, int v_current_min, int v_current_max, float v_speed, int v_min, int v_max, const char* format, const std::optional<std::string>& format_max, ImGuiSliderFlags flags) {
        int *v_current_min_ = &v_current_min;
int *v_current_max_ = &v_current_max;
const char *format_max_ = format_max.has_value() ? format_max.c_str() : nullptr;
        const auto res_ = ImGui::DragIntRange2(label, v_current_min_, v_current_max_, v_speed, v_min, v_max, format, format_max_, flags);
        return std::make_tuple(res_, v_current_min, v_current_max);
    }
    , py::arg("label"), py::arg("v_current_min"), py::arg("v_current_max"), py::arg("v_speed") = 1.0f, py::arg("v_min") = 0, py::arg("v_max") = 0, py::arg("format") = "%d", py::arg("format_max") = std::nullopt, py::arg("flags") = 0, "Creates a draggable integer range slider with the specified label, current min/max values, speed, minimum/maximum values, format strings, and optional flags. Returns true if the values are changed.");
m.def("SliderFloat", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a slider for floating-point values with the specified label, value, min/max range, format string, and optional flags. Returns true if the value is changed.");
m.def("SliderFloat2", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a slider for two floating-point values with the specified label, values, min/max range, format string, and optional flags. Returns true if the values are changed.");
m.def("SliderFloat3", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a slider for three floating-point values with the specified label, values, min/max range, format string, and optional flags. Returns true if the values are changed.");
m.def("SliderFloat4", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a slider for four floating-point values with the specified label, values, min/max range, format string, and optional flags. Returns true if the values are changed.");
m.def("SliderAngle", 
    ...
    , py::arg("label"), py::arg("v_rad"), py::arg("v_degrees_min") = -360.0f, py::arg("v_degrees_max") = +360.0f, py::arg("format") = "%.0f deg", py::arg("flags") = 0, "Creates a slider for angles with the specified label, value in radians, min/max range in degrees, format string, and optional flags. Returns true if the value is changed.");
m.def("SliderInt", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%d", py::arg("flags") = 0, "Creates a slider for integer values with the specified label, value, min/max range, format string, and optional flags. Returns true if the value is changed.");
m.def("SliderInt2", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%d", py::arg("flags") = 0, "Creates a slider for two integer values with the specified label, values, min/max range, format string, and optional flags. Returns true if the values are changed.");
m.def("SliderInt3", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%d", py::arg("flags") = 0, "Creates a slider for three integer values with the specified label, values, min/max range, format string, and optional flags. Returns true if the values are changed.");
m.def("SliderInt4", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%d", py::arg("flags") = 0, "Creates a slider for four integer values with the specified label, values, min/max range, format string, and optional flags. Returns true if the values are changed.");
m.def("VSliderFloat", 
    [](const char* label, const ImVec2& size, float v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags) {
        float *v_ = &v;
        const auto res_ = ImGui::VSliderFloat(label, size, v_, v_min, v_max, format, flags);
        return std::make_tuple(res_, v);
    }
    , py::arg("label"), py::arg("size"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a vertical slider for floating-point values with the specified label, size, value, min/max range, format string, and optional flags. Returns true if the value is changed.");
m.def("VSliderInt", 
    ...
    , py::arg("label"), py::arg("size"), py::arg("v"), py::arg("v_min"), py::arg("v_max"), py::arg("format") = "%d", py::arg("flags") = 0, "Creates a vertical slider for integer values with the specified label, size, value, min/max range, format string, and optional flags. Returns true if the value is changed.");
m.def("InputText", 
    ...
    , py::arg("label"), py::arg("str"), py::arg("flags") = 0, "Creates a text input field with the specified label, buffer, buffer size, optional flags, callback, and user data. Returns true if the text is changed.");
m.def("InputTextMultiline", 
    ...
    , py::arg("label"), py::arg("str"), py::arg("size") = ImVec2(0, 0), py::arg("flags") = 0, "Creates a multiline text input field with the specified label, buffer, buffer size, size, optional flags, callback, and user data. Returns true if the text is changed.");
m.def("InputTextWithHint", 
    ...
    , py::arg("label"), py::arg("hint"), py::arg("str"), py::arg("flags") = 0, "Creates a text input field with a hint, using the specified label, hint, buffer, buffer size, optional flags, callback, and user data. Returns true if the text is changed.");
m.def("InputFloat", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("step") = 0.0f, py::arg("step_fast") = 0.0f, py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a text input field for floating-point values with the specified label, value, optional step, fast step, format string, and flags. Returns true if the value is changed.");
m.def("InputFloat2", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a text input field for two floating-point values with the specified label, values, format string, and optional flags. Returns true if the values are changed.");
m.def("InputFloat3", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a text input field for three floating-point values with the specified label, values, format string, and optional flags. Returns true if the values are changed.");
m.def("InputFloat4", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("format") = "%.3f", py::arg("flags") = 0, "Creates a text input field for four floating-point values with the specified label, values, format string, and optional flags. Returns true if the values are changed.");
m.def("InputInt", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("step") = 1, py::arg("step_fast") = 100, py::arg("flags") = 0, "Creates a text input field for integer values with the specified label, value, optional step, fast step, and flags. Returns true if the value is changed.");
m.def("InputInt2", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("flags") = 0, "Creates a text input field for two integer values with the specified label, values, and optional flags. Returns true if the values are changed.");
m.def("InputInt3", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("flags") = 0, "Creates a text input field for three integer values with the specified label, values, and optional flags. Returns true if the values are changed.");
m.def("InputInt4", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("flags") = 0, "Creates a text input field for four integer values with the specified label, values, and optional flags. Returns true if the values are changed.");
m.def("InputDouble", 
    ...
    , py::arg("label"), py::arg("v"), py::arg("step") = 0.0, py::arg("step_fast") = 0.0, py::arg("format") = "%.6f", py::arg("flags") = 0, "Creates a text input field for double values with the specified label, value, optional step, fast step, format string, and flags. Returns true if the value is changed.");
m.def("ColorEdit3", 
    ...
    , py::arg("label"), py::arg("col"), py::arg("flags") = 0, "Edits an RGB color value with an optional set of flags. Returns true if the color was changed.");
m.def("ColorEdit4", 
    ...
    , py::arg("label"), py::arg("col"), py::arg("flags") = 0, "Edits an RGBA color value with an optional set of flags. Returns true if the color was changed.");
m.def("ColorPicker3", 
    [](const char* label, std::array<float, 3> col, ImGuiColorEditFlags flags) {
    ...
    , py::arg("label"), py::arg("col"), py::arg("flags") = 0, "Displays a color picker for an RGB color value with an optional set of flags. Returns true if the color was changed.");
m.def("ColorPicker4", 
    [](const char* label, std::array<float, 4> col, ImGuiColorEditFlags flags, const std::optional<std::array<float, 4>>& ref_col) {
        const float * ref_col_ = ref_col ? &ref_col.value()[0] : nullptr;
        const auto res_ = ImGui::ColorPicker4(label, col.data(), flags, ref_col_);
        return std::make_tuple(res_, col);
    }
    , py::arg("label"), py::arg("col"), py::arg("flags") = 0, py::arg("ref_col") = std::nullopt, "Displays a color picker for an RGBA color value with an optional set of flags and reference color. Returns true if the color was changed.");
m.def("ColorButton", &ImGui::ColorButton, py::arg("desc_id"), py::arg("col"), py::arg("flags") = 0, py::arg("size") = ImVec2(0, 0), "Displays a color square/button with the specified color, size, and flags. Returns true if the button was pressed.");
m.def("SetColorEditOptions", &ImGui::SetColorEditOptions, py::arg("flags"), "Sets the color edit options, typically called during application startup. Allows selecting a default format, picker type, etc.");
m.def("TreeNode", 
    [](const char* label) {
    ...
    , py::arg("label"), "Creates a tree node with the specified label. Returns true if the node is open, in which case TreePop() should be called to close it.");
m.def("TreeNodeEx", 
    ...
    , py::arg("label"), py::arg("flags") = 0, "Creates an extended tree node with the specified label and flags. Returns true if the node is open.");
m.def("TreePush", 
    [](const char* str_id) {
...
m.def("TreePop", &ImGui::TreePop, "Pops the last element from the tree node stack, decreasing the indentation level.");
m.def("GetTreeNodeToLabelSpacing", &ImGui::GetTreeNodeToLabelSpacing, "Returns the horizontal distance preceding the label when using TreeNode() or Bullet().");
m.def("CollapsingHeader", 
    [](const char* label, ImGuiTreeNodeFlags flags) {
        
        const auto res_ = ImGui::CollapsingHeader(label, flags);
        return res_;
    }
    , py::arg("label"), py::arg("flags") = 0, "Creates a collapsible header with the specified label and flags. Returns true if the header is open.");
m.def("CollapsingHeader", 
    ...
    , py::arg("label"), py::arg("p_visible"), py::arg("flags") = 0, "Creates a collapsible header with the specified label, visibility flag, and flags. Returns true if the header is open.");
m.def("SetNextItemOpen", &ImGui::SetNextItemOpen, py::arg("is_open"), py::arg("cond") = 0, "Sets the next TreeNode or CollapsingHeader open state.");
m.def("Selectable", 
    ...
    , py::arg("label"), py::arg("selected") = false, py::arg("flags") = 0, py::arg("size") = ImVec2(0, 0), "Creates a selectable item with the specified label, selection state, flags, and size. Returns true if the item is clicked.");
m.def("Selectable", 
    ...
    , py::arg("label"), py::arg("p_selected"), py::arg("flags") = 0, py::arg("size") = ImVec2(0, 0), "Creates a selectable item with the specified label, pointer to the selection state, flags, and size. Returns true if the item is clicked.");
m.def("IsItemToggledSelection", &ImGui::IsItemToggledSelection, "Checks if the selection state of the last item was toggled. Useful for retrieving per-item information before reaching EndMultiSelect.");
m.def("BeginListBox", &ImGui::BeginListBox, py::arg("label"), py::arg("size") = ImVec2(0, 0), "Opens a framed scrolling region for a list box with the specified label and size.");
m.def("EndListBox", &ImGui::EndListBox, "Ends the list box opened by BeginListBox. Should be called only if BeginListBox returned true.");
m.def("ListBox", 
    [](const char* label, int current_item, const std::vector<std::string>& items, int items_count, int height_in_items) {
        int *current_item_ = &current_item;
const auto items_ = convert_string_items(items);
        const auto res_ = ImGui::ListBox(label, current_item_, items_.data(), items_count, height_in_items);
        return std::make_tuple(res_, current_item);
    }
    , py::arg("label"), py::arg("current_item"), py::arg("items"), py::arg("items_count"), py::arg("height_in_items") = -1, "Displays a list box with the specified label, current item index, items array, item count, and optional height in items. Returns true if the current item was changed.");
m.def("Value", 
...
m.def("Value", 
    [](const char* prefix, float v, const std::optional<std::string>& float_format) {
        const char *float_format_ = float_format.has_value() ? float_format.c_str() : nullptr;
        ImGui::Value(prefix, v, float_format_);
        
    }
    , py::arg("prefix"), py::arg("v"), py::arg("float_format") = std::nullopt, "Displays a floating-point value with a prefix in the format \"prefix: value\". Optionally specify the format for the float.");
m.def("BeginMenuBar", &ImGui::BeginMenuBar, "Appends to the menu bar of the current window. Requires ImGuiWindowFlags_MenuBar flag to be set on the parent window.");
...
m.def("EndMainMenuBar", &ImGui::EndMainMenuBar, "Ends the main menu bar. Should be called only if BeginMainMenuBar() returned true.");
m.def("BeginMenu", &ImGui::BeginMenu, py::arg("label"), py::arg("enabled") = true, "Creates a sub-menu entry. Should call EndMenu() if this returns true.");
m.def("EndMenu", &ImGui::EndMenu, "Ends the menu created by BeginMenu(). Should be called only if BeginMenu() returned true.");
m.def("MenuItem", 
    [](const char* label, const std::optional<std::string>& shortcut, bool selected, bool enabled) {
        const char *shortcut_ = shortcut.has_value() ? shortcut.c_str() : nullptr;
        const auto res_ = ImGui::MenuItem(label, shortcut_, selected, enabled);
        return res_;
    }
    , py::arg("label"), py::arg("shortcut") = std::nullopt, py::arg("selected") = false, py::arg("enabled") = true, "Creates a menu item with an optional shortcut, selection state, and enabled state. Returns true when activated.");
m.def("MenuItem", 
    ...
    , py::arg("label"), py::arg("shortcut"), py::arg("p_selected"), py::arg("enabled") = true, "Creates a menu item with an optional shortcut and enabled state, and toggles the selection state if p_selected is not NULL. Returns true when activated.");
m.def("BeginTooltip", &ImGui::BeginTooltip, "Begins a tooltip window. Should call EndTooltip() if this returns true.");
m.def("EndTooltip", &ImGui::EndTooltip, "Ends the tooltip window. Should be called only if BeginTooltip() or BeginItemTooltip() returned true.");
m.def("SetTooltip", 
    [](const char * text) {
...
m.def("BeginItemTooltip", &ImGui::BeginItemTooltip, "Begins a tooltip window if the preceding item was hovered. Should call EndTooltip() if this returns true.");
m.def("SetItemTooltip", 
    [](const char * text) {
...
m.def("BeginPopup", &ImGui::BeginPopup, py::arg("str_id"), py::arg("flags") = 0, "Begins a popup window with the specified ID and flags. Should call EndPopup() if this returns true.");
m.def("BeginPopupModal", 
    [](const char* name, std::optional<bool> open, ImGuiWindowFlags flags) {
        bool *p_open = open.has_value() ? &open.value() : nullptr;
        const auto res_ = ImGui::BeginPopupModal(name, p_open, flags);
        return std::make_tuple(res_, open);
    }
    , py::arg("name"), py::arg("open") = std::nullopt, py::arg("flags") = 0, "Begins a modal popup window with the specified name and flags. Should call EndPopup() if this returns true.");
m.def("EndPopup", &ImGui::EndPopup, "Ends a popup window. Should be called only if BeginPopup() or BeginPopupModal() returned true.");
m.def("OpenPopup", 
...
m.def("OpenPopup", 
    ...
    , py::arg("id"), py::arg("popup_flags") = 0, "Marks the popup with the specified ID as open, facilitating calling from nested stacks.");
m.def("OpenPopupOnItemClick", 
    [](const std::optional<std::string>& str_id, ImGuiPopupFlags popup_flags) {
        const char *str_id_ = str_id.has_value() ? str_id.c_str() : nullptr;
        ImGui::OpenPopupOnItemClick(str_id_, popup_flags);
        
    }
    , py::arg("str_id") = std::nullopt, py::arg("popup_flags") = 1, "Helper to open a popup when clicked on the last item. Defaults to right mouse button click.");
m.def("CloseCurrentPopup", &ImGui::CloseCurrentPopup, "Manually closes the current popup.");
m.def("BeginPopupContextItem", 
    [](const std::optional<std::string>& str_id, ImGuiPopupFlags popup_flags) {
        const char *str_id_ = str_id.has_value() ? str_id.c_str() : nullptr;
        const auto res_ = ImGui::BeginPopupContextItem(str_id_, popup_flags);
        return res_;
    }
    , py::arg("str_id") = std::nullopt, py::arg("popup_flags") = 1, "Opens and begins a popup when clicked on the last item. Returns true if the popup is open.");
m.def("BeginPopupContextWindow", 
    [](const std::optional<std::string>& str_id, ImGuiPopupFlags popup_flags) {
        const char *str_id_ = str_id.has_value() ? str_id.c_str() : nullptr;
        const auto res_ = ImGui::BeginPopupContextWindow(str_id_, popup_flags);
        return res_;
    }
    , py::arg("str_id") = std::nullopt, py::arg("popup_flags") = 1, "Opens and begins a popup when clicked on the current window. Returns true if the popup is open.");
m.def("BeginPopupContextVoid", 
    [](const std::optional<std::string>& str_id, ImGuiPopupFlags popup_flags) {
        const char *str_id_ = str_id.has_value() ? str_id.c_str() : nullptr;
        const auto res_ = ImGui::BeginPopupContextVoid(str_id_, popup_flags);
        return res_;
    }
    , py::arg("str_id") = std::nullopt, py::arg("popup_flags") = 1, "Opens and begins a popup when clicked in a void area (where there are no windows). Returns true if the popup is open.");
m.def("IsPopupOpen", &ImGui::IsPopupOpen, py::arg("str_id"), py::arg("flags") = 0, "Returns true if the popup with the specified ID is open.");
m.def("BeginTable", &ImGui::BeginTable, py::arg("str_id"), py::arg("columns"), py::arg("flags") = 0, py::arg("outer_size") = ImVec2(0.0f, 0.0f), py::arg("inner_width") = 0.0f, "Begins a table with the specified number of columns and options. Returns true if the table is successfully created.");
m.def("EndTable", &ImGui::EndTable, "Ends the table created by BeginTable(). Should be called only if BeginTable() returned true.");
m.def("TableNextColumn", &ImGui::TableNextColumn, "Appends into the next column (or the first column of the next row if currently in the last column). Returns true when the column is visible.");
m.def("TableSetColumnIndex", &ImGui::TableSetColumnIndex, py::arg("column_n"), "Sets the current column index for the next item in the table.");
m.def("TableSetupColumn", &ImGui::TableSetupColumn, py::arg("label"), py::arg("flags") = 0, py::arg("init_width_or_weight") = 0.0f, py::arg("user_id") = 0, "Sets up a column in a table. You can provide a label, flags, initial width or weight, and an optional user ID.");
m.def("TableSetupScrollFreeze", &ImGui::TableSetupScrollFreeze, py::arg("cols"), py::arg("rows"), "Freezes the specified number of columns and rows, keeping them visible when the table is scrolled.");
m.def("TableHeader", &ImGui::TableHeader, py::arg("label"), "Submits a single header cell manually, rarely used.");
m.def("TableHeadersRow", &ImGui::TableHeadersRow, "Submits a row with header cells based on data provided to TableSetupColumn(). Also submits the context menu.");
m.def("TableAngledHeadersRow", &ImGui::TableAngledHeadersRow, "Submits a row with angled headers for columns marked with the ImGuiTableColumnFlags_AngledHeader flag. This must be the first row.");
...
m.def("TableGetColumnName", &ImGui::TableGetColumnName, py::arg("column_n") = -1, "Returns the name of the column specified by column_n. Pass -1 to use the current column.");
m.def("TableGetColumnFlags", &ImGui::TableGetColumnFlags, py::arg("column_n") = -1, "Returns the flags associated with the specified column, or the current column if column_n is -1.");
m.def("TableSetColumnEnabled", &ImGui::TableSetColumnEnabled, py::arg("column_n"), py::arg("v"), "Changes the enabled/disabled state of a column. Set to false to hide the column.");
m.def("TableGetHoveredColumn", &ImGui::TableGetHoveredColumn, "Returns the index of the hovered column, or -1 if no column is hovered.");
m.def("TableSetBgColor", &ImGui::TableSetBgColor, py::arg("target"), py::arg("color"), py::arg("column_n") = -1, "Sets the background color for a cell, row, or column. See ImGuiTableBgTarget_ flags for details.");
m.def("Columns", 
    [](int count, const std::optional<std::string>& id, bool border) {
        const char *id_ = id.has_value() ? id.c_str() : nullptr;
        ImGui::Columns(count, id_, border);
        
    }
    , py::arg("count") = 1, py::arg("id") = std::nullopt, py::arg("border") = true, "Legacy columns API. Sets up a number of columns. Use Tables instead for new implementations.");
m.def("NextColumn", &ImGui::NextColumn, "Moves to the next column in the current row, or to the first column of the next row if the current row is finished.");
m.def("GetColumnIndex", &ImGui::GetColumnIndex, "Returns the current column index in the legacy Columns API.");
m.def("GetColumnWidth", &ImGui::GetColumnWidth, py::arg("column_index") = -1, "Returns the width of the specified column in pixels, or the current column if column_index is -1.");
m.def("SetColumnWidth", &ImGui::SetColumnWidth, py::arg("column_index"), py::arg("width"), "Sets the width of the specified column in pixels.");
m.def("GetColumnOffset", &ImGui::GetColumnOffset, py::arg("column_index") = -1, "Gets the position of the column line in pixels from the left side of the content region.");
m.def("SetColumnOffset", &ImGui::SetColumnOffset, py::arg("column_index"), py::arg("offset_x"), "Sets the position of the column line in pixels from the left side of the content region.");
m.def("GetColumnsCount", &ImGui::GetColumnsCount, "Returns the number of columns in the legacy Columns API.");
m.def("BeginTabBar", &ImGui::BeginTabBar, py::arg("str_id"), py::arg("flags") = 0, "Begins a tab bar. Returns true if the tab bar is successfully created.");
m.def("EndTabBar", &ImGui::EndTabBar, "Ends the tab bar created by BeginTabBar(). Should be called only if BeginTabBar() returned true.");
m.def("BeginTabItem", 
    ...
    , py::arg("label"), py::arg("open") = std::nullopt, py::arg("flags") = 0, "Begins a tab item. Returns true if the tab is selected.");
m.def("EndTabItem", &ImGui::EndTabItem, "Ends the tab item created by BeginTabItem(). Should be called only if BeginTabItem() returned true.");
m.def("TabItemButton", &ImGui::TabItemButton, py::arg("label"), py::arg("flags") = 0, "Creates a tab that behaves like a button. Returns true when clicked.");
m.def("SetTabItemClosed", &ImGui::SetTabItemClosed, py::arg("tab_or_docked_window_label"), "Notifies the TabBar or Docking system of a closed tab or window ahead of time. This is useful to reduce visual flicker on reorderable tab bars.");
m.def("LogToTTY", &ImGui::LogToTTY, py::arg("auto_open_depth") = -1, "Starts logging output to the terminal (stdout).");
m.def("LogToFile", 
    [](int auto_open_depth, const std::optional<std::string>& filename) {
        const char *filename_ = filename.has_value() ? filename.c_str() : nullptr;
        ImGui::LogToFile(auto_open_depth, filename_);
        
    }
    , py::arg("auto_open_depth") = -1, py::arg("filename") = std::nullopt, "Starts logging output to a file. If filename is NULL, the log is written to 'imgui_log.txt'.");
m.def("LogToClipboard", &ImGui::LogToClipboard, py::arg("auto_open_depth") = -1, "Starts logging output to the OS clipboard.");
...
m.def("BeginDragDropSource", &ImGui::BeginDragDropSource, py::arg("flags") = 0, "Starts a drag-and-drop source. If this returns true, you should call SetDragDropPayload() and EndDragDropSource().");
m.def("SetDragDropPayload", 
...
const auto data_size = PyBytes_Size(data.ptr());
    ...
    , py::arg("type"), py::arg("data"), py::arg("cond") = 0, "Sets the payload data for the current drag-and-drop operation. The type is a user-defined string.");
m.def("EndDragDropSource", &ImGui::EndDragDropSource, "Ends a drag-and-drop source operation. Should be called only if BeginDragDropSource() returns true.");
m.def("BeginDragDropTarget", &ImGui::BeginDragDropTarget, "Marks an item as a possible drag-and-drop target. If this returns true, you can call AcceptDragDropPayload() and EndDragDropTarget().");
m.def("AcceptDragDropPayload", 
    ...
    , py::arg("type"), py::arg("flags") = 0, "Accepts the drag-and-drop payload if it matches the specified type. Returns the payload data if accepted.");
m.def("EndDragDropTarget", &ImGui::EndDragDropTarget, "Ends a drag-and-drop target operation. Should be called only if BeginDragDropTarget() returns true.");
m.def("GetDragDropPayload", 
    ...
    , "Peeks directly into the current drag-and-drop payload from anywhere. Returns NULL if drag-and-drop is inactive.");
m.def("BeginDisabled", &ImGui::BeginDisabled, py::arg("disabled") = true, "Disables user interactions and dims item visuals. Can be nested.");
m.def("EndDisabled", &ImGui::EndDisabled, "Ends the disabled section started by BeginDisabled().");
m.def("PushClipRect", &ImGui::PushClipRect, py::arg("clip_rect_min"), py::arg("clip_rect_max"), py::arg("intersect_with_current_clip_rect"), "Pushes a clipping rectangle onto the stack. Mouse hovering is affected by this call.");
m.def("PopClipRect", &ImGui::PopClipRect, "Pops the last clipping rectangle from the stack.");
m.def("SetItemDefaultFocus", &ImGui::SetItemDefaultFocus, "Sets the last item as the default focused item of a window.");
m.def("SetKeyboardFocusHere", &ImGui::SetKeyboardFocusHere, py::arg("offset") = 0, "Focuses the keyboard on the next widget. Positive offset can be used to access sub-components, and -1 to access the previous widget.");
m.def("SetNextItemAllowOverlap", &ImGui::SetNextItemAllowOverlap, "Allows the next item to overlap with others.");
m.def("IsItemHovered", &ImGui::IsItemHovered, py::arg("flags") = 0, "Checks if the last item is hovered and usable. Can be customized with ImGuiHoveredFlags.");
m.def("IsItemActive", &ImGui::IsItemActive, "Checks if the last item is active (e.g., button being held, text field being edited).");
m.def("IsItemFocused", &ImGui::IsItemFocused, "Checks if the last item is focused for keyboard or gamepad navigation.");
m.def("IsItemClicked", &ImGui::IsItemClicked, py::arg("mouse_button") = 0, "Checks if the last item is hovered and clicked with the specified mouse button.");
m.def("IsItemVisible", &ImGui::IsItemVisible, "Checks if the last item is visible (i.e., not clipped or scrolled out of view).");
m.def("IsItemEdited", &ImGui::IsItemEdited, "Checks if the last item modified its underlying value or was pressed during this frame.");
m.def("IsItemActivated", &ImGui::IsItemActivated, "Checks if the last item was just made active (previously inactive).");
m.def("IsItemDeactivated", &ImGui::IsItemDeactivated, "Checks if the last item was just made inactive (previously active). Useful for Undo/Redo patterns.");
m.def("IsItemDeactivatedAfterEdit", &ImGui::IsItemDeactivatedAfterEdit, "Checks if the last item was just made inactive and modified its value while active. Useful for Undo/Redo patterns.");
m.def("IsItemToggledOpen", &ImGui::IsItemToggledOpen, "Checks if the last item's open state was toggled (e.g., TreeNode()).");
...
m.def("GetItemRectMin", &ImGui::GetItemRectMin, "Returns the upper-left bounding rectangle of the last item in screen space.");
m.def("GetItemRectMax", &ImGui::GetItemRectMax, "Returns the lower-right bounding rectangle of the last item in screen space.");
...
m.def("IsRectVisible", 
    ...
    , py::arg("size"), "Checks if a rectangle (starting from the cursor position) of the given size is visible and not clipped.");
m.def("IsRectVisible", 
    ...
    , py::arg("rect_min"), py::arg("rect_max"), "Checks if a rectangle defined by rect_min and rect_max is visible and not clipped.");
m.def("GetTime", &ImGui::GetTime, "Returns the global ImGui time, incremented by io.DeltaTime every frame.");
m.def("GetStyleColorName", &ImGui::GetStyleColorName, py::arg("idx"), "Returns a string representing the enum value of a style color.");
m.def("ColorConvertRGBtoHSV", 
...
m.def("IsKeyPressed", &ImGui::IsKeyPressed, py::arg("key"), py::arg("repeat") = true, "Checks if a key was pressed (transitioned from not pressed to pressed). If repeat is true, considers io.KeyRepeatDelay / KeyRepeatRate.");
m.def("IsKeyReleased", &ImGui::IsKeyReleased, py::arg("key"), "Checks if a key was released (transitioned from pressed to not pressed).");
m.def("IsKeyChordPressed", &ImGui::IsKeyChordPressed, py::arg("key_chord"), "Checks if a key chord (combination of modifiers and a key) was pressed.");
m.def("GetKeyPressedAmount", &ImGui::GetKeyPressedAmount, py::arg("key"), py::arg("repeat_delay"), py::arg("rate"), "Returns the number of times a key has been pressed considering the provided repeat rate and delay.");
m.def("GetKeyName", &ImGui::GetKeyName, py::arg("key"), "[DEBUG] Returns the English name of the key.");
m.def("SetNextFrameWantCaptureKeyboard", &ImGui::SetNextFrameWantCaptureKeyboard, py::arg("want_capture_keyboard"), "Overrides the io.WantCaptureKeyboard flag for the next frame.");
m.def("Shortcut", &ImGui::Shortcut, py::arg("key_chord"), py::arg("flags") = 0, "Submits a shortcut route, and returns true if the shortcut is currently active and routed.");
m.def("SetNextItemShortcut", &ImGui::SetNextItemShortcut, py::arg("key_chord"), py::arg("flags") = 0, "Sets the shortcut for the next item.");
m.def("SetItemKeyOwner", &ImGui::SetItemKeyOwner, py::arg("key"), "Sets the key owner to the last item ID if it is hovered or active.");
m.def("IsMouseDown", &ImGui::IsMouseDown, py::arg("button"), "Checks if a mouse button is being held down.");
m.def("IsMouseClicked", &ImGui::IsMouseClicked, py::arg("button"), py::arg("repeat") = false, "Checks if a mouse button was clicked (transitioned from not pressed to pressed).");
m.def("IsMouseReleased", &ImGui::IsMouseReleased, py::arg("button"), "Checks if a mouse button was released (transitioned from pressed to not pressed).");
m.def("IsMouseDoubleClicked", &ImGui::IsMouseDoubleClicked, py::arg("button"), "Checks if a mouse button was double-clicked.");
m.def("GetMouseClickedCount", &ImGui::GetMouseClickedCount, py::arg("button"), "Returns the number of successive clicks of a mouse button.");
m.def("IsMouseHoveringRect", &ImGui::IsMouseHoveringRect, py::arg("r_min"), py::arg("r_max"), py::arg("clip") = true, "Checks if the mouse is hovering a given bounding rectangle in screen space.");
m.def("IsMousePosValid", 
...
m.def("IsMouseDragging", &ImGui::IsMouseDragging, py::arg("button"), py::arg("lock_threshold") = -1.0f, "Checks if the mouse is dragging (moving while holding a button down).");
m.def("GetMouseDragDelta", &ImGui::GetMouseDragDelta, py::arg("button") = 0, py::arg("lock_threshold") = -1.0f, "Returns the delta (change in position) from the initial click position while dragging.");
m.def("ResetMouseDragDelta", &ImGui::ResetMouseDragDelta, py::arg("button") = 0, "Resets the mouse drag delta for the specified button.");
...
m.def("SetNextFrameWantCaptureMouse", &ImGui::SetNextFrameWantCaptureMouse, py::arg("want_capture_mouse"), "Overrides the io.WantCaptureMouse flag for the next frame.");
m.def("GetClipboardText", &ImGui::GetClipboardText, "Returns the text currently in the clipboard.");
m.def("LoadIniSettingsFromDisk", &ImGui::LoadIniSettingsFromDisk, py::arg("ini_filename"), "Loads ini settings from the specified file.");
m.def("SaveIniSettingsToDisk", &ImGui::SaveIniSettingsToDisk, py::arg("ini_filename"), "Saves ini settings to the specified file.");
...
void bind_imgui_enums(py::module& m) {
    ...
    .value("ImGuiTableRowFlags_None", ImGuiTableRowFlags_::ImGuiTableRowFlags_None).value("ImGuiTableRowFlags_Headers", ImGuiTableRowFlags_::ImGuiTableRowFlags_Headers)
    .export_values();
    

    py::enum_<ImGuiTableBgTarget_>(m, "ImGuiTableBgTarget")
    .value("ImGuiTableBgTarget_None", ImGuiTableBgTarget_::ImGuiTableBgTarget_None).value("ImGuiTableBgTarget_RowBg0", ImGuiTableBgTarget_::ImGuiTableBgTarget_RowBg0).value("ImGuiTableBgTarget_RowBg1", ImGuiTableBgTarget_::ImGuiTableBgTarget_RowBg1).value("ImGuiTableBgTarget_CellBg", ImGuiTableBgTarget_::ImGuiTableBgTarget_CellBg)
    .export_values();
